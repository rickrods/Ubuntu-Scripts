...
# Commit the above changes to the system
sysctl -p
...
iptables -t mangle -A PREROUTING -p tcp -m conntrack --ctstate NEW -m tcpmss ! --mss 536:655
...
### THESE LAST 2 I HAVE NOT USED LIKELY THEY WILL NOT BE TOO HELPFUL BUT WHO KNOWS
...
# Rules for a stateful iptables firewall
...
# Allow establish connections to continue working
sudo iptables -A OUTPUT -o eth0 -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
# Allow outbound on port 26656
sudo iptables -A OUTPUT -o eth0 -p tcp -m multiport --ports 26656 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
